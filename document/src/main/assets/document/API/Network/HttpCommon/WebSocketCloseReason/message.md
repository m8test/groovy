...
+ 在8080端口开启一个 WebSocket
  服务器,通过 [WebSocketCloseReason.message()](/API/Network/HttpCommon/WebSocketCloseReason/README.md?id=message)
  获取关闭原因消息.
...
client.webSocket("ws://localhost:8080/message") {
    // delegate -> WebSocketSession
    // WebSocketSession.onOpen() 当 WebSocket 第一次成功连接时会调用此闭包
    onOpen {
