...
+ boolean - 如果抽屉处于关闭状态返回true,否则返回false

示例代码:

+ 启动一个Activity, 并通过 [DrawerState.isOpen()](/API/UI/Compose/State/DrawerState/README.md?id=isOpen) 方法判断抽屉是否打开
...
$composeActivity.createView {
    ...
    ModalDrawer {
        ...
        withAggregatedStates((state): ["isOpen"])
        // ModalDrawer.scrimColor() 用于设置 ModalDrawer 抽屉打开时非抽屉部分的颜色
        scrimColor {
            // delegate -> Color
            // DrawerState.isOpen() 用于判断抽屉是否打开
            def isOpen = state.isOpen()
            if (isOpen) {
