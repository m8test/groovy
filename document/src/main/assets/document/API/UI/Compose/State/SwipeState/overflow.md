...
+ float - 可滑动对象被滑动超过其边界的量

示例代码:

+ 启动一个Activity, 并通过 [SwipeState.overflow()](/API/UI/Compose/State/SwipeState/README.md?id=overflow)
  方法获取可滑动对象被滑动超过其边界的量
    ...
    TextButton {
        ...
        content {
            ...
            Text {
                ...
                withAggregatedStates((state): ["overflow"])
                // Text.style() 用于配置文本样式
                style {
                    // delegate -> TextStyle
                    // BackdropScaffoldState.overflow() 用于获取可滑动对象被滑动超过其边界的量
                    def overflowValue = state.overflow()
                    if (overflowValue > 0) {
