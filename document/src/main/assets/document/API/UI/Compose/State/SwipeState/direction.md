...
+ String - 相对于当前 [SwipeState.currentValue()](/API/UI/Compose/State/SwipeState/README.md?id=currentValue)
  ,如果它从左向右或从上到下移动，则为1f;如果它从右向左或从下到上移动,则将为-1f；如果没有滑动或动画正在进行,则为0f。

示例代码:

+ 启动一个Activity, 并通过 [SwipeState.direction()](/API/UI/Compose/State/SwipeState/README.md?id=direction)
  方法获取可滑动对象移动的方向
