...
+ boolean - 如果动画正在运行返回true,否则返回false

示例代码:

+ 启动一个Activity,
  并通过 [SwipeState.isAnimationRunning()](/API/UI/Compose/State/SwipeState/README.md?id=isAnimationRunning)
  方法判断状态当前是否为动画
    ...
    TextButton {
        ...
        content {
            ...
            Text {
                ...
                style {
                    // delegate -> TextStyle
                    // BackdropScaffoldState.isAnimationRunning() 用于判断状态当前是否为动画
                    if (state.isAnimationRunning() == "Concealed") {
