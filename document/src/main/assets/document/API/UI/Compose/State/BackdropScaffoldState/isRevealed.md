...
+ boolean - false表示后层为隐藏状态,true表示后层为显示状态

示例代码:

+ 启动一个Activity,
  并通过 [BackdropScaffoldState.isRevealed()](/API/UI/Compose/State/BackdropScaffoldState/README.md?id=isRevealed)
  方法判断后层是否显示
  ...
  TextButton {
    ...
    content {
      ...
      Text {
        ...
        withAggregatedStates((state): ["isRevealed"])
        // Text.style() 用于配置文本样式
        style {
          // delegate -> TextStyle
          // BackdropScaffoldState.isRevealed() 用于判断后层是否隐藏
          if (state.isRevealed()) {
