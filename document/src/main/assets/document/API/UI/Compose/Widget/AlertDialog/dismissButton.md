...
+ [AlertDialog](/API/UI/Compose/Widget/AlertDialog/README.md) dismissButton(Closure closure)

参数:

+ Closure closure - 设置取消按钮的闭包
    + 参数 - 无
    + delegate -
...
+ 启动一个Activity显示警告对话框,
  并通过 [AlertDialog.dismissButton()](/API/UI/Compose/Widget/AlertDialog/README.md?id=dismissButton)
  方法设置警告对话框取消按钮.
...
def MyAlertDialog = { state ->
    ...
    AlertDialog {
        ...
        // AlertDialog.dismissButton() 用于设置警告对话框取消按钮
        dismissButton {
            // delegate -> Slot
            // Slot.TextButton() 用于设置文本按钮
            TextButton {
