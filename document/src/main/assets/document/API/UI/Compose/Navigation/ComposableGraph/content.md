...
+ [ComposableGraph](/API/UI/Compose/Navigation/ComposableGraph/README.md) content(Closure closure)

参数:

+ Closure closure - 构建内容的闭包
    + 参数
        + [NavBackStackEntry](/API/UI/Compose/Navigation/NavBackStackEntry/README.md) navBackStackEntry -
          保存了返回栈信息的对象
    + delegate -
...
+ [ComposableGraph](/API/UI/Compose/Navigation/ComposableGraph/README.md) - 返回对象本身方便链式调用

示例代码:

+ 启动一个Activity,
  并通过 [ComposableGraph.content()](/API/UI/Compose/Navigation/ComposableGraph/README.md?id=content)
  方法设置可组合项内容
