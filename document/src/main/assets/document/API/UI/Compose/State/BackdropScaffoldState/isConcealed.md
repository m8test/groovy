...
+ boolean - true表示后层为隐藏状态,false表示后层为显示状态

示例代码:

+ 启动一个Activity,
  并通过 [BackdropScaffoldState.isConcealed()](/API/UI/Compose/State/BackdropScaffoldState/README.md?id=isConcealed)
  方法判断后层是否隐藏
  ...
  TextButton {
    ...
    content {
      ...
      Text {
        ...
        withAggregatedStates((state): ["isConcealed"])
        // Text.style() 用于配置文本样式
        style {
          // delegate -> TextStyle
