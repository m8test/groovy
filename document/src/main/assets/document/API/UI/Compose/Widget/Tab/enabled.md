...
+ 设置 [Tab](/API/UI/Compose/Widget/Tab/README.md) 是否启用.

定义:

+ [Tab](/API/UI/Compose/Widget/Tab/README.md) enabled(boolean value)
...
+ 启动一个Activity, 并通过 [Tab.enabled()](/API/UI/Compose/Widget/Tab/README.md?id=enabled)
  方法设置 [Tab](/API/UI/Compose/Widget/Tab/README.md) 是否启用.

```groovy
def MyTab = { state, tabIndex, iconName, tabText, isEnabled ->
    // delegate -> Slot
    // Slot.Tab() 用于创建 Tab 可组合项
    Tab {
        // delegate -> Tab
        // Tab.enabled() 用于设置 Tab 是否可用
        enabled(isEnabled)
...
$composeActivity.createView {
    ...
    TabRow {
        ...
        tabs {
            ...
            MyTab(state, 0, "Home", "主页", true)
            MyTab(state, 1, "Settings", "设置", true)
            MyTab(state, 2, "Favorite", "喜欢", false)
        }
    }
}
