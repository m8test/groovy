...
+ boolean - true表示抽屉为关闭状态,false表示抽屉为非关闭状态

示例代码:

+ 启动一个Activity,
  并通过 [BottomDrawerState.isClosed()](/API/UI/Compose/State/BottomDrawerState/README.md?id=isClosed)
  方法判断抽屉是否关闭
  ...
  BottomDrawer {
    ...
    content {
      ...
      TextButton {
        ...
        // TextButton.onClick() 用于处理点击事件
        onClick {
          // BottomDrawerState.isClosed() 用于判断抽屉是否关闭
          $assert.assertTrue(state.isClosed(), "抽屉为关闭状态")
