...
+ 异步执行多个手势操作,需要无障碍权限(安卓7.0以上)

定义:

+ boolean gesturesAsync(List\<Closure\> closures)

参数:

+ List\<Closure\> closures - 设置手势的闭包
    + 参数 - 无
    + delegate - [Gesture](/API/Automator/Gesture/README.md)
...
+ 启动一个Activity, 并通过 [Automator.gesturesAsync()](/API/Automator/Automator/README.md?id=gesturesAsync)
  方法异步执行多个手势操作

```groovy
def getGesture(int index, bounds) {
    return {
        // delegate -> Gesture
        ...
        points([{
                    ...
                    // delegate -> Point
                    // Point.x() 用于设置横坐标
                    x(bounds.left() + bounds.width() * (index + 1))
                    // Point.y() 用于设置纵坐标
                    y(bounds.top())
                }, {
                    // delegate -> Point
                    ...
                    IconButton {
                        ...
                        onClick {
                            ...
                            coroutineScope.launch({
                                ...
                                def myGestures = []
                                for (i in 0..<3) {
                                    myGestures.push(getGesture(i, bounds))
                                }
                                // Automator.gesturesAsync() 用于异步执行多个手势操作
                                $automator.gesturesAsync(myGestures)
                            }
                        }
                    }
                }
