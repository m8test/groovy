...
+ 回收图片资源,所有图片资源会在脚本运行结束时被自动回收,所以可以无需调用此方法也可以.但是如果脚本中大量读取图片,那么就需要手动释放不适用的图片节省资源.

定义:

+ [ScriptImage](/API/Image/ScriptImage/README.md) recycle()
...
+ [ScriptImage](/API/Image/ScriptImage/README.md) - 返回对象本身方便链式调用

示例代码:

+ 启动一个Activity, 并通过 [ScriptImage.recycle()](/API/Image/ScriptImage/README.md?id=recycle) 方法回收图片资源
