...
+ [ImageReader](/API/Image/ImageReader/README.md) bytes(byte[] bytes)

参数:

+ byte[] bytes - 图片对应的字节数组
...
+ 启动一个Activity, 并通过 [ImageReader.bytes()](/API/Image/ImageReader/README.md?id=bytes) 方法从字节数组中读取图片
                        ...
                        onClick {
                            ...
                            coroutineScope.launch({
                                ...
                                def originImage = $images.read {
                                    ...
                                    // ImageReader.bytes() 用于从字节数组中读取图片
                                    bytes($images.read { url("https://s1.ax1x.com/2023/02/17/pSbOeMj.jpg") }.toBytes("png", 100))
                                }
                                // Images.read() 用于读取图片
                                def templateImage = $images.read {
                                    // delegate -> ImageReader
                                    // ImageReader.bytes() 用于从字节数组中读取图片
                                    bytes($images.read { url("https://s1.ax1x.com/2023/02/17/pSbOOwq.png") }.toBytes("png", 100))
                                }
