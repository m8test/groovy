...
+ boolean removeOnActivityChangeListener(int listenerId)
...
+ 启动一个Activity,
  并通过 [Accessibility.removeOnActivityChangeListener()](/API/Accessibility/Accessibility/README.md?id=removeOnActivityChangeListener)
  方法移除Activity改变监听器

```groovy
// Accessibility.addOnActivityChangeListener() 用于添加Activity改变监听器
def result = $accessibility.addOnActivityChangeListener { message ->
    $console.log(message)
            ...
            TopAppBar {
                ...
                title {
                    ...
                    Text {
                        ...
                        text("removeOnActivityChangeListener")
                    }
                }
            ...
            Button {
                ...
                content {
                    ...
                    Text {
                        // delegate -> Text
                        // Text.text() 用于设置文本可组合项内容
                        text("移除Activity改变监听器")
                    }
                }
                // Button.onClick() 用于处理 Button 可组合项的点击事件
                onClick {
                    // Accessibility.removeOnActivityChangeListener() 用于移除Activity改变监听器
                    $console.log($accessibility.removeOnActivityChangeListener(result))
                }
            }
        }
    }
