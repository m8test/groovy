...
定义:
+ boolean evaluateJsFromFile(ScriptFile path, boolean repeatable, Closure closure)

参数:
+ ScriptFile path - 需要执行的js文件路径
...
示例代码:
+ 启动一个Activity, 通过 WebViewBridge.evaluateJsFromFile() 方法在 WebView 端执行js文件
 */
