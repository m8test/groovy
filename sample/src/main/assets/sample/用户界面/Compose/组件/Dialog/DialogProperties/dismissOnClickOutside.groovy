...
/*
功能:
+ 设置点击对话框外面是否触发 Dialog.onDismissRequest().

定义:
+ DialogProperties dismissOnClickOutside(boolean value)

参数:
+ boolean value - true表示点击对话框外面可以触发 Dialog.onDismissRequest() ,false表示点击对话框外面不会触发 Dialog.onDismissRequest()

返回值:
+ DialogProperties - 返回对象本身方便链式调用

示例代码:
+ 启动一个Activity显示对话框, 并通过 DialogProperties.dismissOnClickOutside() 方法设置点击对话框外面触发 Dialog.onDismissRequest().
 */
def MyDialog = { state ->
    // delegate -> Slot
    // Slot.Dialog() 用于设置对话框
