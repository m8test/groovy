package sample.用户界面.Compose.组件.SnackBarHost
...
定义:
+ SnackBarHost snackBar(Closure closure)

参数:
+ Closure closure - 设置 SnackBar 的闭包
    + 参数 - SnackBarData
    + delegate - Slot(Modifier)

返回值:
+ SnackBarHost - 返回对象本身方便链式调用

示例代码:
+ 启动一个Activity, 并通过 SnackBarHost.snackBar() 方法设置 SnackBar 可组合项
 */
