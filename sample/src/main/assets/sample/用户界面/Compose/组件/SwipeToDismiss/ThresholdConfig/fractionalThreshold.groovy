package sample.用户界面.Compose.组件.SwipeToDismiss.ThresholdConfig
...
定义:
+ ThresholdConfig fractionalThreshold(float threshold)

参数:
+ float threshold - 阈值,取值范围[0.0-1.0]

返回值:
+ ThresholdConfig - 返回对象本身方便链式调用

示例代码:
+ 启动一个Activity, 并通过 ThresholdConfig.fractionalThreshold() 方法设置百分比阈值
 */
