...
/*
功能:
+ 设置 RangeSlider 值改变结束时监听事件,此回调不应用于更新滑块值(为此使用 onValueChange),而是
  知道用户何时通过结束拖动或单击来完成选择新值。

定义:
+ RangeSlider onValueChangeFinished(Closure closure)
...
示例代码:
+ 启动一个Activity, 并通过 RangeSlider.onValueChangeFinished() 方法设置 RangeSlider 值改变结束时监听事件
 */
