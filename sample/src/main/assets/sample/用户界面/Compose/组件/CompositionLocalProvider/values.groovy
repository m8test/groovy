package sample.用户界面.Compose.组件.CompositionLocalProvider
...
定义:
+ CompositionLocalProvider values(List<ProvidedValue> values)

参数:
+ List<ProvidedValue> values - 需要设置的 ProvidedValue

返回值:
+ CompositionLocalProvider - 返回对象本身方便链式调用

示例代码:
+ 启动一个Activity, 并通过 CompositionLocalProvider.values() 方法设置 ProvidedValue
 */
