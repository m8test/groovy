package sample.用户界面.Compose.组件.SwipeToDismiss

/*
功能:
+ 设置 SwipeToDismiss 需要被刷过才能被关闭的阈值

定义:
+ SwipeToDismiss dismissThresholds(String direction, Closure closure)
...
返回值:
+ SwipeToDismiss - 返回对象本身方便链式调用

示例代码:
+ 启动一个Activity, 并通过 SwipeToDismiss.dismissThresholds() 方法设置 SwipeToDismiss 需要被刷过才能被关闭的阈值
 */
