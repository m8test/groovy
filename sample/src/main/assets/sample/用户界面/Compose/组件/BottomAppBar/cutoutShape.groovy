package sample.用户界面.Compose.组件.BottomAppBar

/*
功能:
+ 设置应用程序栏的剪切形状.一般在使用 Scaffold 时同时设置 floatingActionButton() 以及 bottomBar() 时用到此方法.

定义:
+ BottomAppBar cutoutShape(Closure closure)
...
返回值:
+ BottomAppBar - 返回对象本身方便链式调用

示例代码:
+ 启动一个Activity, 并通过 BottomAppBar.cutoutShape() 方法设置应用程序栏的剪切形状.
 */
def MyBottomAppBar = {
    // delegate -> Slot
