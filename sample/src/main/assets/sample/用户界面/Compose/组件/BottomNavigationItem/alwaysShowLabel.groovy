...
/*
功能:
+ 设置 BottomNavigationItem 未选中时是否显示标签

定义:
+ BottomNavigationItem alwaysShowLabel(boolean value)

参数:
+ boolean value - true表示未选中 BottomNavigationItem 时仍然显示标签,false表示选中的 BottomNavigationItem 时才显示标签

返回值:
+ BottomNavigationItem - 返回对象本身方便链式调用

示例代码:
+ 启动一个Activity, 并通过 BottomNavigationItem.alwaysShowLabel() 方法设置 BottomNavigationItem 未选中时是否显示标签
 */
