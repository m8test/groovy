...
定义1:
+ LazyListScope items(int count, Closure key = null, Closure contentType = null, Closure content)

参数1:
+ int count - 添加到列表的项目数量
+ Closure key - 设置项目的稳定且唯一的密钥。不允许对列表中的多个项目使用相同的键。密钥的类型应该可以通过 Android 上的 Bundle 保存。如果传递 null,则列表中的位置将代表键。当您指定键时，滚动位置将根据键保持不变，这意味着如果您在当前可见项目之前添加/删除项目，则具有给定键的项目将保留为第一个可见项目。
    + 参数
        + int index - 索引值
        + int value - 值
    + delegate - 无
...
定义2:
+ LazyListScope items(List<Object> list, Closure key = null, Closure contentType = null, Closure content)
...
定义3:
+ LazyListScope items(MutableStateList list, Closure key = null, Closure contentType = null, Closure content)

参数3:
+ MutableStateList list - 需要添加到列表的数据
+ Closure key - 设置项目的稳定且唯一的密钥。不允许对列表中的多个项目使用相同的键。密钥的类型应该可以通过 Android 上的 Bundle 保存。如果传递 null,则列表中的位置将代表键。当您指定键时，滚动位置将根据键保持不变，这意味着如果您在当前可见项目之前添加/删除项目，则具有给定键的项目将保留为第一个可见项目。
    + 参数
        + int index - 索引值
        + Object value - 值
...
示例代码:
+ 启动一个Activity, 并通过 LazyListScope.items() 方法添加多个项目到列表中
 */
