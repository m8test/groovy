package sample.用户界面.Compose.组件.NavigationRailItem

/*
功能:
+ 设置 NavigationRailItem 是否启用.

定义:
+ NavigationRailItem enabled(boolean value)

参数:
+ boolean value - true表示启用,false表示未启用

返回值:
+ NavigationRailItem - 返回对象本身方便链式调用

示例代码:
+ 启动一个Activity, 并通过 NavigationRailItem.enabled() 方法设置 NavigationRailItem 是否启用.
 */
