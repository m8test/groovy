package sample.用户界面.Compose.组件.Popup.PopupPositionProvider
...
定义:
+ PopupPositionProvider y(Closure closure)

参数:
+ Closure closure - 获取y坐标的闭包
    + 参数
      + Rect anchorBounds - 此弹出窗口锚定到的布局的窗口相对边界
      + Size windowSize - 包含定位布局的窗口的大小
      + String layoutDirection - 锚定布局的布局方向,值为Ltr或者Rtl
      + Size popupContentSize - 弹出窗口内容的大小
    + delegate - 无

返回值:
+ PopupPositionProvider - 返回对象本身方便链式调用

示例代码:
+ 启动一个Activity, 并通过 PopupPositionProvider.y() 方法设置弹出窗口纵坐标的获取方法
 */
