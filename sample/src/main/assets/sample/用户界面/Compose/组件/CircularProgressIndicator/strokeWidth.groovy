package sample.用户界面.Compose.组件.CircularProgressIndicator
...
定义:
+ CircularProgressIndicator strokeWidth(Number strokeWidth)

参数:
+ Number strokeWidth - 进度条宽度,单位dp

返回值:
+ CircularProgressIndicator - 返回对象本身方便链式调用

示例代码:
+ 启动一个Activity, 并通过 CircularProgressIndicator.strokeWidth() 方法设置进度条宽度
 */
