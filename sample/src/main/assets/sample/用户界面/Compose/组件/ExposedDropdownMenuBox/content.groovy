package sample.用户界面.Compose.组件.ExposedDropdownMenuBox
...
定义:
+ ExposedDropdownMenuBox content(Closure closure)

参数:
+ Closure closure - 设置内容的闭包
    + 参数 - 无
    + delegate - ExposedDropdownMenuBoxScopeSlot(ExposedDropdownMenuBoxScopeModifier)

返回值:
+ ExposedDropdownMenuBox - 返回对象本身方便链式调用

示例代码:
+ 启动一个Activity, 并通过 ExposedDropdownMenuBox.content() 方法设置下拉菜单盒子内容.
 */
