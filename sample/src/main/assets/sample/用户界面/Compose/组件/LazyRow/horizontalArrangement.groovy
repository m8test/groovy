package sample.用户界面.Compose.组件.LazyRow
...
定义:
+ LazyRow horizontalArrangement(String arrangement)
...
+ String arrangement - 水平排列方式,可选值有
    ...
    + SpaceAround - 放置子项时，使它们在主轴上均匀分布，包括第一个子项之前和最后一个子项之后的自由空间，但两个连续子项之间存在的空间量的一半。 视觉上：#1##2##3# 用于 LTR，#3##2##1# 用于 RTL
    + SpaceEvenly - 放置子项，使它们在主轴上均匀分布，包括第一个子项之前和最后一个子项之后的自由空间。 视觉上：#1#2#3# 用于 LTR，#3#2#1# 用于 RTL。

返回值:
+ LazyRow - 返回对象本身方便链式调用

示例代码:
+ 启动一个Activity, 并通过 LazyRow.horizontalArrangement() 方法设置列表水平方向排列方式.
 */
