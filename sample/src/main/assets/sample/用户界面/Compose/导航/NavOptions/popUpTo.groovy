...
/*
功能:
+ 在导航之前弹出给定的目的地,例如登录成功后需要把登录界面弹出,这样通过 NavController.popBackStack() 弹出时就不会返回到登录界面了

定义:
+ NavOptions popUpTo(String route, Closure closure)

参数:
+ String route - 需要弹出的目的地
+ Closure closure - 设置 PopUpTo 的闭包
    + 参数 - 无
    + delegate - PopUpTo
...
示例代码:
+ 启动一个Activity, 并通过 NavOptions.popUpTo() 方法在导航之前弹出给定的目的地
 */
