package sample.用户界面.Compose.导航.ComposableGraph
...
定义:
+ ComposableGraph content(Closure closure)

参数:
+ Closure closure - 构建内容的闭包
    + 参数
        + NavBackStackEntry navBackStackEntry - 保存了返回栈信息的对象
    + delegate - Slot(Modifier)

返回值:
+ ComposableGraph - 返回对象本身方便链式调用

示例代码:
+ 启动一个Activity, 并通过 ComposableGraph.content() 方法设置可组合项内容
 */
