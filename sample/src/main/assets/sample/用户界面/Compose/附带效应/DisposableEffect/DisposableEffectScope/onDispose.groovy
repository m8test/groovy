package sample.用户界面.Compose.附带效应.DisposableEffect.DisposableEffectScope

/*
功能:
+ 为 DisposableEffect 在它离开组合或其键更改时运行监听器

定义:
+ DisposableEffectScope onDispose(Closure closure)
...
返回值:
+ DisposableEffectScope - 返回对象本身方便链式调用

示例代码:
+ 启动一个Activity, 并通过 DisposableEffectScope.onDispose() 方法为 DisposableEffect 在它离开组合或其键更改时运行监听器
 */
