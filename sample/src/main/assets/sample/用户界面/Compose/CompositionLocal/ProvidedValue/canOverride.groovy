...
返回值:
+ boolean - true 表示由 ProvidableCompositionLocal.provides() 创建,则提供的值将会覆盖作用域中可能已经存在的值,false 表示由 ProvidableCompositionLocal.providesDefault() 创建,则提供的值将不会覆盖作用域中可能已经存在的值。

示例代码:
+ 启动一个Activity, 并通过 ProvidedValue.canOverride() 方法判断提供的值是否会覆盖作用域中可能已经存在的值
 */
