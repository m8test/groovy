...
/*
功能:
+ 获取布局视口的起始偏移（以像素为单位）。您可以将其视为可见的最小偏移量。通常为0，但如果在应用 beforeContentPadding 为非零，则可以为负，因为内容填充区域中显示的内容仍然可见。 您可以使用它来了解 visibleItemsInfo 中的哪些项目是完全可见的。
...
示例代码:
+ 启动一个Activity, 并通过 LazyListLayoutInfo.viewportStartOffset() 方法获取布局视口的起始偏移（以像素为单位）
 */
