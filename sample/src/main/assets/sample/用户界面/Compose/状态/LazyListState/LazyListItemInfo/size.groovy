package sample.用户界面.Compose.状态.LazyListState.LazyListItemInfo

/*
功能:
+ 获取项目的主轴大小(以像素为单位)。请注意，如果您在项目的可组合槽中发出多个布局，则该大小将作为其大小的总和计算。
...
示例代码:
+ 启动一个Activity, 并通过 LazyListLayoutInfo.size() 方法获取项目的主轴大小(以像素为单位)
 */
            ...
            Button {
                ...
                content {
                    ...
                    Text {
                        ...
                        text("visibleItemsInfo size:" + mutableState.value())
                    }
                }
                ...
                onClick {
                    ...
                    info.each { lazyListItemInfo ->
                        // LazyListItemInfo.size() 用于获取项目的主轴大小(以像素为单位)。
                        infoString += lazyListItemInfo.size() + ","
                    }
