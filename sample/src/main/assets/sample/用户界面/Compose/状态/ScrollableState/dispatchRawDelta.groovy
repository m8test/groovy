...
/*
功能:
+ 以像素为单位调度滚动增量，避免所有与滚动相关的机制。 注意：与 scroll 不同，使用此方法调度任何增量都不会触发嵌套滚动，不会停止正在进行的滚动/拖动动画，并且会绕过任何优先级的滚动。此方法还将忽略reverseDirection和在scrollable中设置的其他参数。该方法在内部用于嵌套滚动调度和其他低级操作，允许ScrollableState的实现者根据自己的需要影响消耗。通过此方法手动调度增量可能会导致用户体验不佳，您必须首选 scroll 方法而不是此方法。
...
示例代码:
+ 启动一个Activity, 并通过 ScrollableState.dispatchRawDelta() 方法以像素为单位调度滚动增量
 */
