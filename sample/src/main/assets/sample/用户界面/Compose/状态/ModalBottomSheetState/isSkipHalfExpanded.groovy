...
/*
功能:
+ 设置是否跳过半展开状态.如果工作表足够高,是否应跳过半展开状态。如果为true,则在以编程方式或通过用户交互隐藏工作表时,工作表将始终展开到“展开”状态并移动到“隐藏”状态。如果 initialValue 为 HalfExpanded, 则不能设置为true。如果为 initialValue 提供了 HalfExpended，则将引发 IllegalArgumentException。

定义:
+ ModalBottomSheetState isSkipHalfExpanded(boolean value)
...
示例代码:
+ 启动一个Activity, 并通过 ModalBottomSheetState.isSkipHalfExpanded() 方法设置是否跳过半展开状态
 */
