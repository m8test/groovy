...
/*
功能:
+ 滚动懒列表,调用此函数以控制滚动并通过 ScrollScope.scrollBy() 获得发送滚动事件的能力。所有更改逻辑滚动位置的操作都必须在滚动块中执行（即使它们没有调用此对象上的任何其他方法），以确保强制执行互斥。如果从其他地方调用 scroll ，则将取消此操作。

定义:
+ Job scroll(String scrollPriority, Closure closure)

参数:
+ String scrollPriority - 对状态执行突变的优先级。遵循枚举类值的自然顺序；与另一个值比较为>的值具有更高的优先级。同等或更高优先级的突变将中断当前正在进行的突变。可选值有
    + Default - 突变的默认优先级。可以被其他 Default、UserInput 或 PreventUserInput 优先级操作中断。默认优先级应用于不应中断用户输入的编程动画或更改。
...
示例代码:
+ 启动一个Activity, 并通过 ScrollableState.scroll() 方法滚动懒列表
 */
