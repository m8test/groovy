package sample.用户界面.Compose.修饰符.ExposedDropdownMenuBoxScopeModifier

/*
功能:
+ 设置 ExposedDropdownMenu 的大小，这将匹配 TextField 的宽度（如果 matchTextFieldWidth 设置为 true）。它还会更改 ExposedDropdownMenu 的高度，因此它会采用尽可能大的高度，以免与 TextField 和软件键盘重叠。

定义:
+ ExposedDropdownMenuBoxScopeModifier exposedDropdownSize(boolean matchTextFieldWidth)

参数:
+ boolean matchTextFieldWidth - 菜单是否应与其所附加的文本字段的宽度相匹配。 如果设置为 true，宽度将匹配文本字段的宽度。

返回值:
+ ExposedDropdownMenuBoxScopeModifier - 返回对象本身方便链式调用

示例代码:
+ 启动一个Activity, 并通过 ExposedDropdownMenuBoxScopeModifier.exposedDropdownSize() 方法设置 ExposedDropdownMenu 的大小
 */
