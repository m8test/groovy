...
/*
功能:
+ 通过将最小宽度设置为等于最大宽度乘以分数. 请注意，默认情况下，分数为 1，因此修饰符将使内容填充整个父级宽度。常规 Modifier.fillMaxWidth() 不能在滚动水平布局内工作.

定义:
+ LazyItemScopeModifier fillParentMaxWidth(float fraction)
...
示例代码:
+ 启动一个Activity, 并通过 LazyItemScopeModifier.fillParentMaxWidth() 方法通过将最小宽度设置为等于最大宽度乘以分数
 */
