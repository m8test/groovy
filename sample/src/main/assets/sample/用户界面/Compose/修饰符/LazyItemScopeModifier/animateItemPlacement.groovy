...
/*
功能:
+ 为惰性列表中的项目放置设置动画。当您通过 LazyListScope.item()/LazyListScope.items() 提供键时，此修饰符将启用项目重新排序动画。 除了项目重新排序之外，由排列或对齐更改等事件引起的所有其他位置更改也将被动画化。

定义:
+ LazyItemScopeModifier animateItemPlacement(FiniteAnimationSpec animationSpec)

参数:
+ FiniteAnimationSpec animationSpec - 将用于为项目放置设置动画的有限动画。

返回值:
+ LazyItemScopeModifier - 返回对象本身方便链式调用

示例代码:
+ 启动一个Activity, 并通过 LazyItemScopeModifier.animateItemPlacement() 方法为惰性列表中的项目放置设置动画
 */
