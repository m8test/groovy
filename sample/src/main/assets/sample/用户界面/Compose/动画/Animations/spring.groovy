...
/*
功能:
+ 创建一个使用给定弹簧常数（即阻尼比和刚度）的 SpringSpec。

定义:
+ SpringSpec spring(float dampingRatio, float stiffness, Object visibilityThreshold)
...
返回值:
+ SpringSpec - 新创建的 SpringSpec 对象

示例代码:
+ 启动一个Activity, 并通过 Animations.spring() 方法创建一个使用给定弹簧常数（即阻尼比和刚度）的 SpringSpec。
 */
