...
/*
功能:
+ 创建 CoroutineScope ,用于在协程中执行耗时操作,防止阻塞UI线程

定义:
+ CoroutineScope newCoroutineScope(Closure closure)

参数:
+ Closure closure - 设置 CoroutineContext 的闭包
    + 参数 - 无
    + delegate - CoroutineContext

返回值:
+ CoroutineScope - 新创建的 CoroutineScope

示例代码:
+ 启动一个Activity显示对话框, 并通过 Slot.newCoroutineScope() 方法创建 CoroutineScope
 */
def MyDialog = { state ->
    // delegate -> Slot
    // Slot.Dialog() 用于设置对话框
