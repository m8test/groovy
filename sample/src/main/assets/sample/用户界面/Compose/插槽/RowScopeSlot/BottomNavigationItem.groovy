package sample.用户界面.Compose.插槽.RowScopeSlot

/*
功能:
+ 创建 BottomNavigationItem

定义:
+ RowScopeSlot BottomNavigationItem(Closure closure)

参数:
+ Closure closure - 设置 BottomNavigationItem 的闭包
    + 参数 - 无
    + delegate - BottomNavigationItem

返回值:
+ RowScopeSlot - 返回对象本身方便链式调用

示例代码:
+ 启动一个Activity, 并通过 RowScopeSlot.BottomNavigationItem() 方法创建 BottomNavigationItem
 */
