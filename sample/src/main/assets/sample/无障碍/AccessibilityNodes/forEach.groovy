package sample.无障碍.AccessibilityNodes
...
定义:
+ AccessibilityNodes forEach(Closure closure)

参数:
+ Closure closure - 用于遍历操作的闭包
    + 参数
        + AccessibilityNode node - 当前遍历的节点
    + delegate - 无

返回值:
+ AccessibilityNodes - 返回对象本身方便链式调用

示例代码:
+ 启动一个Activity, 并通过 AccessibilityNodes.forEach() 方法遍历所有节点
 */
