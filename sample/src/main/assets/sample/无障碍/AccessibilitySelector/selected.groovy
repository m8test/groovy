...
定义:
+ AccessibilitySelector selected(Closure closure)

参数:
+ Closure closure - 设置过滤器的闭包
    + 参数
        + boolean selected - 节点 selected 值
    + delegate - 无
    + 返回值
...
示例代码:
+ 启动一个Activity, 并通过 AccessibilitySelector.selected() 方法设置节点 selected 过滤器
 */
                ...
                actions {
                    ...
                    IconButton {
                        ...
                        onClick {
                            ...
                            // AccessibilitySelector.selected() 用于设置节点 selected 过滤器
                            accessibilitySelector.selected { !it }
