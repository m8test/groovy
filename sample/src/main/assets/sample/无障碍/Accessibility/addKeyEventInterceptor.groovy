...
+ Closure closure - 设置按键事件拦截器的闭包,闭包将会在安卓UI线程执行
    + 参数
        + KeyEvent event - 按键事件对象
    + delegate - 无
    + 返回值 - true表示该按键事件已经被处理,不会继续传递(也就是事件被拦截了),false表示事件继续传递,由其他事件处理器或者系统处理

返回值:
+ int - 成功时为拦截器id,失败则为-1

示例代码:
+ 启动一个Activity, 并通过 Accessibility.addKeyEventInterceptor() 方法添加按键事件拦截器
 */
