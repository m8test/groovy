...
/*
功能:
+ 判断协程是否处于活跃状态,一般用于配合协程的取消. 使用协程执行CPU密集任务时,为了能让协程正常被取消,需要通过此方法判断当前协程是否处于活跃状态,如果不处于活跃状态应该需要停止执行任务实现协程的取消,否则一直执行CPU密集型任务会导致协程无法被取消.
...
示例代码:
+ 通过 CoroutineScope.isActive() 判断协程是否处于活跃状态.
 */
