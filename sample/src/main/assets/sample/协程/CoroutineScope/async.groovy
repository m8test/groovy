...
/*
功能:
+ 启动一个子协程,可以获得协程执行的结果(如果CoroutineContext设置了Job参数,那么就是启动一个新协程,与启动它的协程没有父子关系)

定义:
+ Deferred async(Closure context = null, String start = null, Closure runnable)
...
返回值:
+ Deferred - 可获取协程执行结果的对象.

示例代码:
+ 通过 CoroutineScope.async() 启动两个子协程并获取结果.
 */
