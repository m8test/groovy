...
+ 取消协程(通过抛出CancellationException实现)以及其子协程,需要注意以下几点:
    + 协程的取消需要内部配合,使用 CoroutineScope.isActive() 判断是否处于活动状态或者使用 CoroutineScope.delay() 代替 Threads.sleep()
    + 不要打破协程的父子结构,也就是尽量不要使用 CoroutineContext.job() 配置 Job 类型, 如果非要使用,最好只在 Coroutines.newScope() 中配置,不要在其他地方配置
    + 如果捕获 CancellationException 需要重新抛出来,否则无法实现取消功能

定义:
+ CoroutineScope cancel(String message = null)
