...
功能:
+ 处理协程未捕获的异常
    + 协程构建器(协程创建)有两种形式: 自动传播异常(CoroutineScope.launch())或向用户暴露异常(CoroutineScope.async())。前者这类构建器(CoroutineScope.launch())将异常视为未捕获异常,而后者(CoroutineScope.async())则依赖用户来最终消费异常,可以通过捕获 Deferred.await() 方法(消费)抛出的异常.
    + CoroutineExceptionHandler 仅在未捕获的异常(CoroutineScope.launch())上调用,并且只有当这些构建器用于创建一个根协程时,即该协程不是另一个协程的子协程时才会被调用.
    + 因为所有子协程(在另一个 Job 上下文中创建的协程)委托它们的父协程处理它们的异常,然后它们也委托给其父协程,以此类推直到根协程.因此永远不会使用在子协程上下文中设置的 CoroutineExceptionHandler。
    + 除此之外, CoroutineScope.async() 构建器始终会捕获所有异常并将其表示在结果 Deferred 对象中,因此它的 CoroutineExceptionHandler 也无效。
    + 需要注意的是,你无法从 CoroutineExceptionHandler 的异常中恢复.当调用处理者的时候,协程已经完成并带有相应的异常.通常,该处理器用于记录异常,显示某种错误消息.

定义:
+ CoroutineContext exceptionHandler(Closure handler)

参数:
+ Closure handler - 异常处理器
    + 参数
        + CoroutineContext context - 异常协程的协程上下文
        + Throwable e - 未处理的异常
    + delegate - 无
...
示例代码:
+ 通过 CoroutineContext.exceptionHandler() 配置协程异常处理器.
 */
