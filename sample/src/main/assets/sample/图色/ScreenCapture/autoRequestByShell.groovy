package sample.图色.ScreenCapture
...
定义:
+ ScreenCapture autoRequestByShell(boolean autoRequestByShell)

参数:
+ boolean autoRequestByShell - true表示自动通过 Shell 授予截图权限,false表示不通过 Shell 授予截图权限

返回值:
+ ScreenCapture - 返回对象本身方便链式调用

示例代码:
+ 启动一个Activity, 并通过 ScreenCapture.autoRequestByShell() 方法设置是否自动通过 Shell 授予截屏权限
 */
