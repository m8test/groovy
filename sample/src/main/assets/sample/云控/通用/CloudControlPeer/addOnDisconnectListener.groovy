...
定义:
+ int addOnDisconnectListener(Closure closure)

参数:
+ Closure closure - 设置监听器的闭包
    + 参数
        + CloudControlDisconnection - 接收到数据的连接
    + delegate - 无

返回值:
+ int - 监听器id, CloudControlServer.removeOnDisconnectListener() 取消监听器时传递的参数

示例代码:
+ 通过 CloudControlPeer.addOnDisconnectListener() 添加云控连接断开监听器
 */
