...
定义:
+ boolean removeOnStartListener(int listenerId)
...
示例代码:
+ 通过 CloudControlServer.removeOnStartListener() 移除云控服务器启动监听器
 */
...
if (serverPort != -1) {
    ...
    def listenerId = $cloudControlServer.addOnStartListener {
        // CloudControlServer.port() 用于获取云控服务器启动的端口号
    ...
    // CloudControlServer.removeOnStartListener() 用于移除云控服务器启动监听器
     $cloudControlServer.removeOnStartListener(listenerId)
