...
定义1:
+ ConnectionInfo authorizationCode(String authorizationCode)

参数1:
+ String authorizationCode - 服务器授权码

返回值1:
+ ConnectionInfo - 返回对象本身方便链式调用

示例代码:
+ 通过 ConnectionInfo.authorizationCode() 设置/获取服务器授权码
 */
...
if (isConnected) {
    ...
    $cloudControlClient.addOnConnectListener {
        ...
        // ConnectionInfo.authorizationCode() 用于获取服务器授权码
        $console.log("服务器授权码:" + info.authorizationCode())
    ...
    $cloudControlClient.connect {
        ...
        // ConnectionInfo.authorizationCode() 用于设置服务器授权码
