...
定义1:
+ ConnectionInfo reconnectInterval(long reconnectInterval)

参数1:
+ long reconnectInterval - 重连间隔时间

返回值1:
+ ConnectionInfo - 返回对象本身方便链式调用

示例代码:
+ 通过 ConnectionInfo.reconnectInterval() 设置/获取重连间隔时间
 */
...
if (isConnected) {
    ...
    $cloudControlClient.addOnConnectListener {
        ...
        // ConnectionInfo.reconnectInterval() 用于获取重连间隔时间
        $console.log("重连间隔时间:" + info.reconnectInterval())
    ...
    $cloudControlClient.connect {
        ...
        autoReconnect(true)
        // ConnectionInfo.reconnectInterval() 用于设置重连间隔时间
