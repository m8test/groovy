...
/*
功能:
+ 删除一个 HttpClient ,一般情况下不会用到方法,脚本停止后会自动删除所有由 HttpClientManager.newClient() 创建的所有 HttpClient .

定义:
+ boolean deleteClient(HttpClient httpClient)

参数:
+ HttpClient httpClient - 需要被删除的 HttpClient

返回值:
+ boolean - 删除成功返回 true ,失败返回 false

示例代码:
+ 新建一个 HttpClient 并访问本地服务器, 访问结束后删除该 HttpClient.
 */
