...
返回值:
+ boolean - 如果请求体是multipart类型返回true,否则返回false

示例代码:
+ 在8080端口开启一个服务器,通过 HttpServerRequest.isMultipart() 判断请求体是否为 multipart 类型.
 */
...
def server = $httpServerManager.newServer(8080) {
    ...
    routing {
        ...
        post("/isMultipart") {
            ...
            def request = request()
            // HttpServerRequest.path() 可获得访问路径,此处为 "/isMultipart"
            def path = request.path()
            $assert.assertTrue(path == "/isMultipart")
            $console.log("Path->$path")
            // HttpServerRequest.isMultipart() 用于判断请求体是否为 multipart 类型
            def isMultipart = request.isMultipart()
            if (isMultipart) {
                $console.log("请求体是multipart类型")
...
def response = client.request("http://localhost:8080/isMultipart") {
...
// 断言返回结果为"/isMultipart"
$assert.assertTrue(textResult == "/isMultipart", "返回结果为 /isMultipart")
client.request("http://localhost:8080/isMultipart") {
    // delegate -> HttpClientRequest
    // HttpClientRequest.method() 用于配置请求方法,此处为配置为Get请求.除此之外,还有post,put,patch,delete,head,options等方法
    method "post"
}
