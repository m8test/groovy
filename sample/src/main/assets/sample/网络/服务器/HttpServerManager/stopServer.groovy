...
定义:
+ boolean stopServer(int port)

参数:
+ int port - 需要停止的服务器启动的端口号.
...
$httpServerManager.startServer(8080) {
    ...
    routing {
        ...
        get("/stopServer") {
            ...
            def request = request()
            // HttpServerRequest.path() 可获得访问路径,此处为 "/stopServer"
            def path = request.path()
            $assert.assertTrue(path == "/stopServer")
...
def response = client.request("http://localhost:8080/stopServer") {
...
// 断言返回结果为"/stopServer"
$assert.assertTrue(textResult == "/stopServer", "返回结果为 /stopServer")
